...
rho = pd.read_csv('Dataset/Attack_dataset/rte_case14_realistic/rho_attack_rte_case14_realistic_PLS.csv')
rho = np.array(rho)[:,1:]

corr = np.corrcoef(rho.T)
adjacency_matrix = np.where(np.abs(corr)>adj_th,1,0)
...
file_name = f'Results/Attack/rte_case14_realistic/PLS/test_{test_number}/MSE_log.txt'
...
for i in range(np.shape(y_test)[2]):
    ...
    fig_name = f'Results/Attack/rte_case14_realistic/PLS/test_{test_number}/feature_{i}.png'
    plt.savefig(fig_name)

model.save(f'Results/Attack/rte_case14_realistic/PLS/test_{test_number}/my_model')


df = pd.DataFrame(y_test[:,0,:])
path = os.path.abspath(f'Results/Attack/rte_case14_realistic/PLS/test_{test_number}/y_test.csv')
df.to_csv(path)

df = pd.DataFrame(y_pred[:,0,:])
path = os.path.abspath(f'Results/Attack/rte_case14_realistic/PLS/test_{test_number}/y_pred.csv')
df.to_csv(path)
